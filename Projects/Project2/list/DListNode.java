...
public class DListNode extends ListNode {//implements Comparable<ListNode>{
  ...
  public int compareTo(ListNode node) {
	  if(((Integer)item).intValue() > ((Integer)node.item).intValue()) {
		  return 1;
	  } else if(((Integer)item).intValue() < ((Integer)node.item).intValue()) {
		  return -1;
	  } else {
		  return 0;
	  }
  ...
  public void insertAfter(Object item) throws InvalidNodeException {
    ...
    //   new node.
    DListNode newAdded = new DListNode(item, (DList)myList, this, this.next);
    this.next.prev = newAdded;
    this.next = newAdded;
    myList.size++;
  ...
  public void insertBefore(Object item) throws InvalidNodeException {
    ...
    //   what DList it's in.  You should use myList.newNode() to create the
    //   new node.
    DListNode newAdded = new DListNode(item, (DList)myList, this.prev, this);
    this.prev.next = newAdded;
    this.prev = newAdded;
    myList.size++;
  }
